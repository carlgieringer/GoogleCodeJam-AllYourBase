...
trait GroupPanel extends Panel {
  ...
  def autoCreateGaps_=(b: Boolean) {
    layout.setAutoCreateGaps(b)
  }

  def autoCreateContainerGaps = layout.getAutoCreateContainerGaps

  def autoCreateContainerGaps_=(b: Boolean) {
  ...
  implicit def convertAlignment(a: Alignment.Value): GroupLayout.Alignment = GroupLayout.Alignment.values()(a.id)
  ...
  case class Group(group: GroupLayout#Group) {
    self =>
    def addItem(item: GroupItem) {
      item match {
        case i: AnyGroupItem =>
          i match {
            case ComponentItem(c, SizeRange(Size(min), Size(pref), Size(max))) =>
              group.addComponent(c.peer, min, pref, max)
            case Gap(SizeRange(Size(min), Size(pref), Size(max))) =>
              group.addGap(min, pref, max)
            case Subgroup(g) =>
              group.addGroup(g.group)
          }
        case _ =>
          sys.error("Unhandled GroupItem: " + item)
      }
    }

    def add(items: GroupItem*) {
  ...
  class SequentialGroup(override val group: GroupLayout#SequentialGroup) extends Group(group) {
    ...
    override def addItem(item: GroupItem) {
      item match {
        case i: SeqGroupItem =>
          i match {
            case BaselineComponentItem(c, SizeRange(Size(min), Size(pref), Size(max))) =>
              group.addComponent(true, c.peer, min, pref, max)
            case BaselineSubgroup(g) =>
              group.addGroup(true, g.group)
            case ContainerGap(pref, max) =>
              group.addContainerGap(pref, max)
            case PreferredGap(placement, pref, max) =>
              group.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.values()(placement.id), pref, max)
          }
  ...
  def parallel(align: Alignment.Value, resize: Boolean): P = par(layout.createParallelGroup(align, resize))

  def parallel(): P = parallel(Alignment.Leading, resize = true)

  def parallel(align: Alignment.Value): P = parallel(align, resize = true)

  def baseline(resize: Boolean, anchorTop: Boolean): P = par(layout.createBaselineGroup(resize, anchorTop))

  def honorsVisibility = layout.getHonorsVisibility

  def honorsVisibility_=(b: Boolean) {
    layout.setHonorsVisibility(b)
  }
  ...
  def replace(c1: Component, c2: Component) {
    layout.replace(c1.peer, c2.peer)
  }

  def linkSizes(cs: Component*) {
    layout.linkSize(cs.map(_.peer): _*)
  }

  def linkWidths(cs: Component*) {
    layout.linkSize(javax.swing.SwingConstants.HORIZONTAL, cs.map(_.peer): _*)
  }

  def linkHeights(cs: Component*) {
    layout.linkSize(javax.swing.SwingConstants.VERTICAL, cs.map(_.peer): _*)
  }

  def horizontalGroup(g: Group) {
    layout.setHorizontalGroup(g.group)
  }

  def verticalGroup(g: Group) {
    layout.setVerticalGroup(g.group)
  }
}
